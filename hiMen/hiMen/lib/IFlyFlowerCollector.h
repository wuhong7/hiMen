 ...
 *
 *  @param value 设置YES开启日志收集
 */
+ (void) SetCollectorStatus:(BOOL)value;
...
/**
 *  设置卡顿日志收集状态，默认为关闭
 *  @param blockStatus YES:开启卡顿收集；NO：关闭卡顿收集
 */
+ (void) SetBlockMonitorStatus:(BOOL) blockStatus;

/**
 *  设置卡顿判断时间间隔，默认为5s
 *  @param timeInterval 单位为s ,值大于零
 */
+ (void) SetBlockMonitorTimeInterval:(double) timeInterval;
